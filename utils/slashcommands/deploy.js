...
async function deployAll(client) {
    ...
    try {
        ...
        global.logger.error(`[${client.shard.ids[0] + 1}] Could not register application commands: ${error.stack || error}`);
    }
}
...
async function deployGuild(client, guildId) {
    ...
    try {
        ...
        global.logger.error(`[${client.shard.ids[0] + 1}] Could not register application commands for guild ${guildId}: ${error.stack || error}`);
    }
}
module.exports = {
    deployAll,
